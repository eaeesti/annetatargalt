...
if (!STRAPI_API_TOKEN) {
  ...
  console.error('  3. Create a new token with "Full access" or "Custom" with read permissions');
  console.error('  4. Copy the token and set it as an environment variable');
  console.error('\nUsage:');
  console.error('  STRAPI_API_TOKEN=your-token-here node src/db/migrations/01-export-strapi-data.js');
  process.exit(1);
}
...
async function exportData() {
  ...
  try {
    ...
    console.log(`  - Recurring Donations: ${data.recurringDonations?.length || 0}`);
    console.log(`  - Org Recurring Donations: ${data.organizationRecurringDonations?.length || 0}`);
    console.log(`  - Donations: ${data.donations?.length || 0}`);
    console.log(`  - Organization Donations: ${data.organizationDonations?.length || 0}`);
    console.log(`  - Donation Transfers: ${data.donationTransfers?.length || 0}`);
    ...
    const orgsWithoutInternalId = data.organizations.filter(org => !org.internalId);
    if (orgsWithoutInternalId.length > 0) {
      console.error(`  ✗ WARNING: ${orgsWithoutInternalId.length} organizations missing internalId!`);
      console.error('    Organizations:', orgsWithoutInternalId.map(o => `ID ${o.id}: "${o.title}"`).join(', '));
    } else {
      console.log('  ✓ All organizations have internalId');
    }

    // Check for duplicate internalIds
    const internalIds = data.organizations.map(org => org.internalId).filter(Boolean);
    const duplicates = internalIds.filter((id, index) => internalIds.indexOf(id) !== index);
    if (duplicates.length > 0) {
      console.error(`  ✗ WARNING: Duplicate internalIds found: ${[...new Set(duplicates)].join(', ')}`);
    } else {
    ...
    const invalidRefs = [...orgDonationRefs].filter(ref => !validInternalIds.has(ref));

    if (invalidRefs.length > 0) {
      console.error(`  ✗ WARNING: organizationDonations reference non-existent internalIds: ${invalidRefs.join(', ')}`);
    } else {
      console.log('  ✓ All organizationDonations reference valid organizations');
    }
    ...
    console.error(`  2. The endpoint is accessible at ${STRAPI_URL}/api/donations/export`);
    process.exit(1);
  }
}
